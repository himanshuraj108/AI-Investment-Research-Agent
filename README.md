...
Results are streamed live to the UI via Server-Sent Events (SSE), with each section appearing token-by-token. All research is saved to a PostgreSQL database (Supabase) for history — clicking a past report loads instantly from the DB without re-running the pipeline.
...
| LLM — Research | Groq `llama-3.1-8b-instant` (fast, for 4 research nodes) |
| LLM — Verdict | Groq `llama-3.3-70b-versatile` (high accuracy, for final verdict) |
...
LangGraph StateGraph (5 sequential nodes)
   ├── node_companyInfo    → Tavily search → llama-3.1-8b-instant summarize → SSE stream
   ├── node_financials     → Tavily search → llama-3.1-8b-instant summarize → SSE stream
   ├── node_marketPosition → Tavily search → llama-3.1-8b-instant summarize → SSE stream
   ├── node_risks          → Tavily search → llama-3.1-8b-instant summarize → SSE stream
   └── node_verdict        → llama-3.3-70b-versatile final JSON decision → SSE emit
        ↓
Final state saved to Supabase (full research + verdict + sources)
        ↓
React UI renders progressively — sections stream token-by-token and auto-collapse when done
```
...
The final `node_verdict` receives all four summaries and uses `llama-3.3-70b-versatile` to produce a structured JSON decision with: verdict, confidence score (1–100), executive summary, strengths (×4), weaknesses (×4), and final reasoning.

### Streaming (SSE)
The backend uses Server-Sent Events with the following event types:
- `step_start` → UI activates the spinner for that step
- `token` → individual LLM tokens streamed to the section in real-time
- `step` → full section data + Tavily source URLs
- `verdict` → final INVEST/PASS JSON
- `done` → pipeline complete, history refreshed
- `error` → surfaced cleanly to the user

### API Key Fallback
Both Groq and Tavily support primary + fallback key rotation. If the primary key hits a rate limit (HTTP 429) or auth error (401), the system automatically retries with the fallback key — zero downtime for the user.
...
| **SSE over WebSockets** | Simpler server implementation, native browser `EventSource` API, no extra libraries | One-directional only — fine for this use case |
...
| **Cached DB load from history** | Clicking past reports is instant — no wasted API calls | Data is point-in-time; click Re-run for latest |
...
1. **Real financial data API** — Integrate Yahoo Finance or Alpha Vantage for actual live stock prices and financial statements
2. **PDF export** — One-click report download with all research formatted professionally
3. **Historical comparison** — Chart confidence scores over time for the same company to see sentiment shifts
4. **Sector benchmarking** — Compare the company against its industry peers on key metrics
5. **LangSmith tracing** — Add observability to monitor the AI pipeline in production
6. **Authentication** — User sessions so each user sees only their own research history
7. **Redis caching** — Cache pipeline results for 24 hours to avoid duplicate API calls on popular companies
